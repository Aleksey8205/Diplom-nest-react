...
const SupportChatClient = ({ isOpen, onClose }: IModalProps) => {
...
console.log(request)
  ...
  useEffect(() => {
    if (isOpen) {
      fetch(
        `${API_URL}/api/client/support-requests/?user=${user.user?.id}&isActive=true`,
        ...
        .then((data) => {
          if (data.length > 0) {
            ...
            fetch(`${API_URL}/api/client/support-requests`, {
              method: "POST",
              credentials: "include",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ user: user.user?.id, text: "" }),
            })
              .then((response) => {
                if (response.ok) {
                  return response.json();
  ...
  return (
    <>
      ...
      >
        ...
        <button
          className="button-one chat"
          onClick={closeModalAndDisconnect}
        >
          <X />
        </button>
      </ReactModal>
